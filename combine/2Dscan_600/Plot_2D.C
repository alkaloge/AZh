...
std::map<TString, TGraph*> Find_2D(int nPoints, // sqrt(number_of_points) 
				   ...
				   double dLower, // adjustments
				   double dUpper  // adjustments
				   ) { 
  ...
  limit->Draw("r_bbA:r_ggA>>hist","2*deltaNLL");
  delete dummy;
...
void Plot_2D( ) {
  ...
  TFile * fileExp = new TFile("2D_scan_v2.root");
  TTree * treeExp = (TTree*)fileExp->Get("limit");

  double xmin_frame = -1.5;
  double xmax_frame =  1.5;

  double ymin_frame = -0.3;
  double ymax_frame =  1.5;
  ...
  */
  ...
  leg->SetHeader("m_A = 600 GeV");
    //  leg->AddEntry(graphBest,"best fit","p");
  leg->AddEntry(graphSM,"best fit","p");
